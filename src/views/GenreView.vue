...
function genreFilter(): [string, string] {
  if (props.id.startsWith("name:")) return ["Genres", props.id.slice(5)];
  return ["GenreIds", props.id];
}
...
function baseParams(startIndex = 0): [string, string][] {
  return [
    ...
    ["Fields", "PrimaryImageAspectRatio,Overview,ProductionYear,Genres,GenreItems,UserData"],
    ["SortBy", sortBy.value],
...
<template>
  <main class="genre">
    <GlassNavBar show-back>
      <template #title>{{ genreName }}</template>
      <template #right>
        <div class="sort">
    ...
    <section class="genre__head">
      <p>{{ total }} 部作品</p>
    </section>

    <section class="genre__content">
      <div v-if="loading && items.length === 0" class="genre__empty">
        <Icon icon="lucide:loader" width="22" class="spin" />
        <span>加载中</span>
      </div>
      <div v-else-if="loadError" class="genre__empty genre__empty--error">
        <Icon icon="lucide:triangle-alert" width="22" />
        <span>{{ loadError }}</span>
      </div>
      <div v-else-if="items.length === 0" class="genre__empty">
        <Icon icon="lucide:inbox" width="22" />
        <span>暂无作品</span>
      </div>
      <div v-else ref="scroller" class="genre__grid" @scroll.passive="onScroll">
        <PosterCard
          v-for="item in items"
          :key="item.Id"
          :item="item"
      ...
      <div v-if="items.length > 0 && hasMore" class="genre__more">
