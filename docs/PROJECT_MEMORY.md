...
|---|---|---|
| 登录 / 多线路 / 竞赛 / 测活 / 保号 | ✅ 已实现 | |
| MPV 播放 + 字幕 + 弹幕 | ✅ 已实现 | IPC 仍用 `fd://0`，见差距表 |
| 下载 / 伪装下载 / 边看边下 | ✅ 已实现 | |
| 通知中心 + 托盘 | ✅ 已实现 | |
| 远程控制 / EmbySocket | ✅ 已实现 | |
| 虚拟列表 / 海报懒加载 | ✅ 已实现 | |
| 快捷键 + Windows SMTC | ✅ 已实现 | |
| 收藏页 / 聚合视界 | ⚠️ 视图文件存在，**路由未注册** | |
| MPV 内置打包 | ⚠️ 仅 `fonts.conf`，无 `mpv.exe` | 见差距表 |
| 品牌统一 Hills Lite | ⚠️ 部分字符串仍为 Emby Player | 见差距表 |
...
| 规则 | 目标 | 当前代码 |
|---|---|---|
| 本地 release | **只产出 exe**，不打 msi/nsis | ❌ `tauri.conf.json` 仍 `bundle.active: true`, `targets: all` |
...
| # | 记忆 / 规范目标 | 代码现状 | 优先级 |
|---|---|---|---|
| G1 | 品牌 **Hills Lite** | Sidebar/窗口/title 多为 **Emby Player**；仅 `MpvBanner.vue` 写 Hills Lite | P2 |
| G2 | 主导航含 **收藏 / 聚合视界** | `FavoritesView.vue` / `AggregateView.vue` 存在，**router 未注册** | P2 |
| G3 | MPV IPC 命名管道 `--input-ipc-server` | `ipc.rs` 仍 `--input-ipc-client=fd://0` | P1 |
| G4 | 内置 `resources/mpv/mpv.exe` + build.rs 复制 | 仅 `resources/mpv/mpv/fonts.conf`；`build.rs` 仅 `tauri_build::build()` | P1 |
| G5 | 本地构建只产 exe | `bundle.active: true`, `targets: all` | P2 |
| G6 | `docs/CHANGE_LOG/` 持续维护 | **本次 bootstrap 前为空** | P0 ✅ 正在补 |
| G7 | `PROJECT_MEMORY.md` 主索引 | **本次新建** | P0 ✅ |
| G8 | Player `back()` fire-and-forget stop | 待每次改动时复核 `PlayerView.vue` | P3 |
| G9 | 打开闪退 | 已加 `crash.log`；根因待日志 | P0 |
| G10 | MpvBanner + detectMpv | 组件未挂载；API/命令不存在 | P2 |
...
> ⚠️ 旧版 `CURRENT_STATE.md`（2026-05-24）曾声称 G3/G4/G5 已对齐 — **与磁盘代码不符**，以本表与 AUDIT 为准。
